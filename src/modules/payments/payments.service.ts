...
import { PaidVia, UserRole } from '@prisma/client';
import { CreatePaymentDto } from './dto/create-payment.dto';
import { PrismaService } from 'src/core/database/prisma.service';
import { amountToPenny, getInMills, pennyToAmount, validateWithinMinutes } from 'src/core/utils/time';
import { CACHE_MANAGER } from '@nestjs/cache-manager';
import { TAuthUser } from 'src/common/types/user';
import { PaymeAccountRecord, PaymeErrorCode, PaymeErrorResponse, PaymeMethods, PaymeRequest, PaymeRequestParams, PaymeResponse, PaymeResponses, PaymeTransaction, PaymeTransactionReason, PaymeTransactionState } from 'src/common/types/payments_types/payme';
...
export class PaymentsService {
    ...
    private async checkAccount(account: PaymeAccountRecord, amount: number) {
        ...
        const course = await this.prisma.course.findUnique({
            where: { id: Number(account.course_id), published: true },
            select: {
                id: true,
                price: true,
                name: true,
    ...
    private async validateTransactionTimeout(
        ...
        if (
            ...
            !validateWithinMinutes(new Date(transaction.create_time), this.$transactionTimeout)
        ) {
    ...
    > {
        ...
        if (transaction) {
            ...
            return {
                result: {
                    create_time: getInMills(transaction.create_time ?? new Date),
        ...
        return {
            result: {
                ...
                create_time: getInMills(newTransaction.create_time ?? new Date),
            },
        };
    }
    ...
    > {
        ...
        if (transaction.state !== PaymeTransactionState.CREATED) {
            ...
            return {
                result: {
                    transaction: transaction.id,
                    state: transaction.state,
                    perform_time: getInMills(transaction.perform_time ?? new Date()),
                },
            };
        }
        ...
        if (transaction.userId === null || transaction.courseId === null) {
            return {
                error: {
                    code: PaymeErrorCode.TRANSACTION_NOT_FOUND,
                    message: {
                        en: 'Invalid transaction data',
                        ru: 'Неверные данные транзакции',
                        uz: 'Noto‘g‘ri tranzaksiya ma’lumotlari',
                    },
                },
            };
        }
        ...
        return {
            result: {
                transaction: transaction.id,
                perform_time: getInMills(editedTransaction.perform_time ?? new Date),
    ...
    > {
        ...
        return {
            result: {
                ...
                perform_time: getInMills(transaction.perform_time ?? new Date),
                create_time: getInMills(transaction.create_time ?? new Date),
                cancel_time: getInMills(transaction.cancel_time ?? new Date),
            },
        };
    }
        ...
        if (transaction.state === PaymeTransactionState.CREATED) {
            ...
            return {
                result: {
                    ...
                    cancel_time: getInMills(updatedTransaction.cancel_time ?? new Date),
        ...
        return {
            result: {
                transactions: transactions.map<PaymeTransaction>((transaction) => ({
                    ...
                    time: getInMills(transaction.create_time ?? new Date),
                    create_time: getInMills(transaction.create_time ?? new Date),
                    perform_time: getInMills(transaction.perform_time ?? new Date),
                    cancel_time: getInMills(transaction.cancel_time ?? new Date),
                    account: {
                        course_id: String(transaction.courseId ?? ''),
